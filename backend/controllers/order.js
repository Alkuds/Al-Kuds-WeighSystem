...
const OrderFinishState = async (req, res) => {
  ...
  let newUpdatedOrder = await Order.findOneAndUpdate(
    { _id: orderId },
    { ticket: orderTickets, totalProfit: totalProfitForOrder },
    { returnDocument: "after" }
  );
...
const addOrderStatement = async (req, res) => {
  ...
  try {
    ...
    let walletTransaction = await Wallet.findOneAndUpdate(
      { bankName },
      {
        $push: {
          transactions: {
            amount: amount, // Fixed variable name
            clientId: order.clientId,
            type,
            orderId,
          },
      ...
      { returnDocument: "after", new: true } // Ensures updated document is returned
    );

    if (!walletTransaction || !walletTransaction.transactions.length) {
      return res.status(500).json({ error: "Failed to update wallet" });
    }

    // Update order statement
    const newTransaction =
      walletTransaction.transactions[walletTransaction.transactions.length - 1];
    order.statement.push({
      paidAmount: amount,
      clientId: order.clientId,
      bankName: bankName,
      date: new Date().toLocaleString("en-EG", { timeZone: "Africa/Cairo" }),
      walletId: newTransaction._id,
    });
    ...
    console.error("Error in addOrderStatement:", err);
